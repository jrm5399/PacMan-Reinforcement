...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            ...
            for j, state in enumerate(self.mdp.getStates()):
                #If we arent in a terminal state, compute the best action to take
                if not self.mdp.isTerminal(state):
                    #get the current states actions
                    curr_state_actions = []
                    
                    #Loop through all possible actions we can take from our current state
                    ...
                    #Q2 PLOTTING UPPER LEFT CORNER = (0,2)
                    # if state == (0,2):
                    #     graph.append(round(global_counter[state], 4))

            self.values = global_counter

            #Writing data to CSV
            # with open('Q2plots.csv', 'w', encoding='UTF8') as f:
            #     writer = csv.writer(f)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        discount, values, curr_val, global_vals = self.discount, self.values, 0, []
        ...
        return curr_val if curr_val < best else best
